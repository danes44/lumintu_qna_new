<?php
    ...
    $url = $kel2_api."/items/registration?fields=validated_on,id_participant,id_session&filter[id_session]=".$sesi_id[1]."&filter[id_participant]=".$peserta_id[1];
	$curl = curl_init();
    ...
    $url_sesi = $kel1_api."/items/session?fields=session_id,start_time,finish_time&filter[session_id]=".$sesi_id[1];
    $curl = curl_init();
    ...
    if (empty($hasil["data"])){
        ...
        } else if ( new DateTime("2021-12-02T13:00:00") >= new DateTime($jam_selesai)){
